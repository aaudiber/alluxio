...
public class InodeStoreBench {
  ...
  private static final AtomicInteger sNextInodeId = new AtomicInteger(1);
  ...
  private static void writeBenchmark() throws InterruptedException {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      List<Callable<Void>> threads = IntStream.range(0, numThreads).mapToObj(x -> (Callable<Void>) () -> {
        count.addAndGet(doForMs(timeMs, InodeStoreBench::writeInode, barrier));
        return null;
      }).collect(Collectors.toList());
      service.invokeAll(threads);
      System.out.printf("Performed %d operations using %d threads in %dms%n", count.get(), numThreads, timeMs);
    }
    service.shutdownNow();
  }
  ...
  private static void writeInode() {
    int id = sNextInodeId.getAndIncrement();
